...
interface ClinicReviewFormProps {
  clinic: ClinicInsert & { id: string; images: ClinicImage[] };
  states: ClinicState[];
  areas: ClinicArea[];
  hours: ClinicHours[];
...
export default function FormEditClinic({
  ...
  const [orderedImages, setOrderedImages] = useState<ClinicImageEntry[]>(() =>
    clinicImagesToEntries(clinic.images || []),
  );
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (updatedClinic) {
        const savedImages = await persistClinicImageOrder(
          supabase,
          updatedClinic.id,
          orderedImages,
